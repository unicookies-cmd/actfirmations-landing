...
(function () {
  /* ---------------------------
     Cookie meta (UI & Tracking Only)
  --------------------------- */
  const COOKIE_META = {
    "confetti-sparkle": { name: "Confetti Sparkle" },
    "incognidough":     { name: "Incognidough" },
    "sunburst-love":    { name: "Sunburst Love" },
    "moo-moon":         { name: "Moo Moon" },
    "starry-night":     { name: "Starry Night" },
    "banana-blossom":   { name: "Banana Blossom" },
    "smore-unity":      { name: "S'more Unity" }
  };
  ...
  const MASTER_MESSAGES = [
    ...
    "You matter more than you realize - and today, you are appreciated.",
    ...
    "You do not have to earn belonging - you already have it.",
    ...
    "Your effort is felt, even when it is quiet.",
  ...
  const HOUSE_BLEND = MASTER_MESSAGES.map(clean).filter(Boolean);

  const COOKIE_MESSAGES = {};
  
  // Every cookie type gets the same full randomized pool
  Object.keys(COOKIE_META).forEach((cookieId) => {
    COOKIE_MESSAGES[cookieId] = HOUSE_BLEND;
  });

  // Export to window for app.js to pick up
